		...
		if len(REF) > 1 or len(ALT) > 1 or ALT == "*" :  # exlcude indels
			...
			if ALT == ".": # if no ALT allele ( = monorphic site)	
				...
				if arrline[6] != "PASS" : # exclude lowQ SNP
					...
					gtPos = findPosInFORMATStr(arrline[8], "GT") #FORMAT=<ID=FT,Number=.,Type=String,Description="Genotype-level filter">
						...
						if len(arrInd) < int(covPos+1): # if individual doesn't have DP field
							...
							if cov >= mincov[nameInd[i]] and cov <= maxcov[nameInd[i]]:
								if arrInd[gtPos] == "0/0" or arrInd[gtPos] == "0|0":
									...
									site.append(ALT)
									continue
								elif arrInd[gtPos] == "0/1" or arrInd[gtPos] == "0|1":
									#print(pos,"-",nameInd[i],"- kept adding a ALT=",ALT)
									site.append(REF)
									site.append(ALT)
									continue
								elif arrInd[gtPos] == "1/0" or arrInd[gtPos] == "1|0":
									#print(pos,"-",nameInd[i],"- kept adding a ALT=",ALT)
									site.append(ALT)
									site.append(REF)
									continue
								else: #mask potential other situations
									site.append("N")
									site.append("N")
