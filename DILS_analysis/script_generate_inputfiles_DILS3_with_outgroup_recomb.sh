#!/bin/bash
#SBATCH -p workq
# TL - 200723 # MAJ 130624 to select only non-recombining regions
...
# MAJ: 130624 to subsample recombining region
#1/ subsample recombining
grep "Candidate_recombining" $rootdir/regions_potentially_non_recombining_recombing.txt > $rootdir/regions_potentially_recombining.tmp
rm $rootdir/"$Species1"_list_genomic_regions_recombining.txt
while read line; do
	locus=$(echo "$line" | awk -F "\t" '{print $2}')
	grep "$locus" $rootdir/"$Species1"_list_genomic_regions.txt >> $rootdir/"$Species1"_list_genomic_regions_recombining.txt
done < $rootdir/regions_potentially_recombining.tmp
...
#done
shuf -n "$nbloci" $rootdir/"$Species1"_list_genomic_regions_recombining.txt > $rootdir/"$Species1"_list_genomic_regions.subset"$nbloci".txt
...
new_rootdir=$(echo "/work/genphyse/cytogen/Thibault/SeqApiPop_demography/DILS3_candidaterecombining")
cd $new_rootdir
