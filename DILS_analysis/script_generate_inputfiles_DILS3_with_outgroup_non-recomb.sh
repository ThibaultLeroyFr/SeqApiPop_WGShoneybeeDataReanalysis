#!/bin/bash
#SBATCH -p workq
# TL - 200723 # MAJ 130624 to select only non-recombining regions # MAJ030724 for removing the few closely related individuals (DILS3)
...
Species2=$(echo "Caucasia") #e.g. Ligustica
Outgroup=$(echo "Laboriosa") #e.g. Cerana
nbloci=$(echo "50" | bc)
nbloci_to_sample=$(echo "35" | bc) # normally 50-16 chr but one chromosome without any potential non-recombining regions based on the criteria used
...
for i in $rootdir/$Species2/*.bed.tmp; do
        cd $i
        ls *.fasta | grep -v "$species2""2" | grep -v "clean" > list_tmp
        while read line; do
...
# MAJ: 130624 to subsample non-recombining region
#1/ subsample non-recombining
grep "Candidate_non-recombining" $rootdir/regions_potentially_non_recombining_recombing.txt > $rootdir/regions_potentially_non_recombining.tmp
rm $rootdir/"$Species1"_list_genomic_regions_nonrecombining.txt
while read line; do
	locus=$(echo "$line" | awk -F "\t" '{print $2}')
	grep "$locus" $rootdir/"$Species1"_list_genomic_regions.txt >> $rootdir/"$Species1"_list_genomic_regions_nonrecombining.txt
done < $rootdir/regions_potentially_non_recombining.tmp

#2/ subsample 1 locus per chromosome
rm $rootdir/"$Species1"_list_genomic_regions.subset"$nbloci".txt
for chr in {"NC_037638","NC_037639","NC_037640","NC_037641","NC_037642","NC_037643","NC_037644","NC_037645","NC_037646","NC_037647","NC_037648","NC_037649","NC_037650","NC_037651","NC_037652","NC_037653"}; do 
	grep "$chr" $rootdir/"$Species1"_list_genomic_regions_nonrecombining.txt | shuf -n 1 >> $rootdir/"$Species1"_list_genomic_regions.subset"$nbloci".txt
done
shuf -n "$nbloci_to_sample" $rootdir/"$Species1"_list_genomic_regions_nonrecombining.txt >> $rootdir/"$Species1"_list_genomic_regions.subset"$nbloci".txt
# 3/ remove sample windows if exists
uniq $rootdir/"$Species1"_list_genomic_regions.subset"$nbloci".txt > $rootdir/"$Species1"_list_genomic_regions.subset"$nbloci".tmp
mv $rootdir/"$Species1"_list_genomic_regions.subset"$nbloci".tmp $rootdir/"$Species1"_list_genomic_regions.subset"$nbloci".txt
...
new_rootdir=$(echo "/work/genphyse/cytogen/Thibault/SeqApiPop_demography/DILS3_candidatenonrecombining")
cd $new_rootdir
